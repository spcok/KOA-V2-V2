...
export function SignGenerator({ animal, orgProfile, onClose }: SignGeneratorProps) {
  ...
  const fetchContent = () => {
      if (!navigator.onLine) {
          toast.error("Offline: AI Signage Generation disabled.");
          return;
      }
      ...
      startTransition(async () => {
          try {
              ...
              setContent({ 
                  diet: data.mainContent?.diet || [],
                  habitat: data.mainContent?.habitat || [],
                  didYouKnow: data.mainContent?.didYouKnow || [],
                  speciesBrief: data.brief || '',
                  wildOrigin: data.mainContent?.wildOrigin || '',
                  speciesStats: data.mainContent?.speciesStats || { ...DEFAULT_STATS }
              });
  ...
  // AUDIT PATCH: Safely cast incoming database dates to Strings before reading them
                            ...
                            <div className="mb-6 flex justify-between items-start pt-2">
                                ...
                                <div className="flex items-start gap-6">
                                     <div className="w-36 h-36 rounded-xl overflow-hidden border-4 border-white shadow-lg bg-slate-100 shrink-0">
                                         <img src={animal.image_url !== '-1' ? animal.image_url : ''} alt={animal.name} className="w-full h-full object-cover" crossOrigin="anonymous"/>
                                     </div>
                            ...
                            <div className="grid grid-cols-12 gap-8 flex-1">
                                <div className="col-span-7 flex flex-col gap-5">
                                     <div>
                                         ...
                                         <p className="text-sm font-bold text-slate-600 italic leading-snug">{Array.isArray(animal.description) && animal.description[0] !== 'none' ? animal.description.join(' ') : "No description recorded."}</p>
                                     </div>
                                     <div className={`${theme.containerBg} border ${theme.border} p-4 rounded-xl relative group flex-1`}>
                                         ...
                                         {isEditingText ? (
                                             <EditTextArea value={content.speciesBrief || ''} onChange={(val) => setContent(prev => ({ ...prev, speciesBrief: val }))} className="h-full bg-white/50" />
                                         ) : (
                                     ...
                                     <div className="grid grid-cols-2 gap-x-8 gap-y-4 mt-auto">
                                         <div><h4 className="text-[9px] font-black text-slate-300 uppercase tracking-widest">ANIMAL AGE</h4><p className="font-black text-slate-800">{getAge(animal.date_of_birth)}</p></div>
                                         <div><h4 className="text-[9px] font-black text-slate-300 uppercase tracking-widest">SEX</h4><p className="font-black text-slate-800 uppercase">{animal.gender}</p></div>
                                         <div>
                                             <h4 className="text-[9px] font-black text-slate-300 uppercase tracking-widest">NATIVE ORIGIN</h4>
                                             {isEditingText ? <EditInput value={content.wildOrigin || ''} onChange={(val) => setContent(prev => ({ ...prev, wildOrigin: val }))} /> : <p className="font-black text-slate-800 text-sm truncate">{content.wildOrigin || 'Generating...'}</p>}
                                         </div>
                                         {(animal.category === 'Mammal' || animal.category === 'Exotic') ? (
                                             ...
                                             <div><h4 className="text-[9px] font-black text-slate-300 uppercase tracking-widest">CHIP/RING</h4><p className="font-black text-slate-800 font-mono text-sm">{animal.microchip_id !== 'unknown' ? animal.microchip_id : animal.ring_number !== 'unknown' ? animal.ring_number : 'N/A'}</p></div>
                                         )}
                                     </div>
                                </div>
                                     ...
                                     <div className="flex flex-col gap-3">
                                         <div className="flex gap-3">
                                             <div className="flex-1 bg-orange-500 text-white p-3 rounded-xl shadow-lg flex items-center gap-3">
                                                 ...
                                                 <div className="min-w-0"><p className="text-[8px] font-black opacity-80 uppercase tracking-widest whitespace-nowrap">DAY TARGET</p><p className="text-2xl font-black leading-none">{animal.target_day_temp_c !== -1 ? animal.target_day_temp_c : '?'}°C</p></div>
                                             </div>
                                             <div className="flex-1 bg-emerald-600 text-white p-3 rounded-xl shadow-lg flex items-center gap-3">
                                                 ...
                                                 <div className="min-w-0"><p className="text-[8px] font-black opacity-80 uppercase tracking-widest whitespace-nowrap">NIGHT TARGET</p><p className="text-2xl font-black leading-none">{animal.target_night_temp_c !== -1 ? animal.target_night_temp_c : '?'}°C</p></div>
                                             </div>
                                         </div>
                                         {(animal.target_humidity_min_percent !== -1 || animal.target_humidity_max_percent !== -1) && (
                                            <div className="bg-cyan-600 text-white p-3 rounded-xl shadow-lg flex items-center gap-3">
                                                 <Droplets size={24} className="shrink-0" />
                                                 <div className="min-w-0"><p className="text-[8px] font-black opacity-80 uppercase tracking-widest whitespace-nowrap">HUMIDITY RANGE</p><p className="text-2xl font-black leading-none">{animal.target_humidity_min_percent !== -1 ? animal.target_humidity_min_percent : '?'}-{animal.target_humidity_max_percent !== -1 ? animal.target_humidity_max_percent : '?'}%</p></div>
                                            </div>
                                         )}
                                     </div>
                                </div>
                        ...
                        <>
                            ...
                            <div className="flex-1 py-8 pl-5 pr-8 grid grid-cols-12 gap-6 overflow-hidden">
                                <div className={`col-span-4 flex flex-col ${mapLayout === 'bottom' ? 'gap-2' : 'gap-4'}`}>
                                    <div className="aspect-[3/4] w-full rounded-[1.5rem] overflow-hidden border-4 border-[#1e293b] shadow-xl relative shrink-0"><img src={animal.image_url !== '-1' ? animal.image_url : ''} alt={animal.name} className="w-full h-full object-cover" crossOrigin="anonymous"/></div>
                                    ...
                                    {mapLayout === 'side' ? (
                                        <div className="bg-slate-50 rounded-2xl p-4 border-2 border-slate-200 shadow-sm flex flex-col items-center flex-1 min-h-0">
                                            ...
                                            <div className="rounded-xl overflow-hidden border border-slate-200 w-full bg-white relative flex items-center justify-center flex-1">{animal.distribution_map_url && animal.distribution_map_url !== '-1' ? <img src={animal.distribution_map_url} alt="Range Map" className="w-full h-full object-cover" crossOrigin="anonymous"/> : <div className="w-full h-full flex items-center justify-center text-slate-300"><Globe size={48} /></div>}</div>
                                        </div>
                                    ) : (
                                        <>
                                            <div className="flex flex-col gap-3 bg-slate-50 p-4 rounded-xl border border-slate-200 shadow-sm">
                                                ...
                                                <div className="flex items-center gap-3 border-b border-slate-200 pb-2"><div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center text-slate-500 shadow-sm border border-slate-200 shrink-0"><Info size={16}/></div><div><p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">SEX</p><p className="font-bold text-slate-800 text-sm">{animal.gender}</p></div></div>
                                        ...
                                        {mapLayout === 'side' && (
                                            <>
                                                <div className="flex gap-8 mb-6 mt-6 bg-slate-50 p-3 rounded-xl border border-slate-100">
                                                    ...
                                                    <div className="flex items-center gap-3"><div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center text-slate-500 shadow-sm border border-slate-200"><Info size={20}/></div><div><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">SEX</p><p className="font-bold text-slate-800">{animal.gender}</p></div></div>

...
export function DailyRoundsView() {
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            animalQuery += " ORDER BY display_order ASC, name ASC";

            const animalRes = await db.query(animalQuery, params);
            
            // O(N) Hierarchical Mob Grouping
            const parentMap = new Map<string, any[]>();
            const roots: any[] = [];
            
            animalRes.rows.forEach(a => {
                if (!a.parent_mob_id || a.parent_mob_id === '00000000-0000-0000-0000-000000000000') {
                    roots.push(a);
                } else {
                    if (!parentMap.has(a.parent_mob_id)) parentMap.set(a.parent_mob_id, []);
                    parentMap.get(a.parent_mob_id)!.push(a);
                }
            });
            
            const groupedAnimals: any[] = [];
            const groupedIds = new Set<string>();

            roots.forEach(root => {
                groupedAnimals.push(root);
                groupedIds.add(root.id);
                if (parentMap.has(root.id)) {
                    parentMap.get(root.id)!.forEach(child => {
                        groupedAnimals.push(child);
                        groupedIds.add(child.id);
                    });
                }
            });

            // Catch any orphans (children whose parents were filtered out by the active tab)
            animalRes.rows.forEach(a => {
                if (!groupedIds.has(a.id)) {
                    groupedAnimals.push(a);
                    groupedIds.add(a.id);
                }
            });
            ...
            return { animals: groupedAnimals, checks: checksMap };
        }
    });
    ...
    const totalAnimals = animals.filter(a => a.entity_type === 'INDIVIDUAL' || a.entity_type === 'MOB').length;
    ...
    const toggleMutation = useMutation({
        mutationFn: async ({ animalId, field, value, note }: any) => {
            ...
            if (existing) {
                ...
                await db.query(`INSERT INTO daily_rounds (${cols.join(', ')}) VALUES (${cols.map((_, i) => i === 6 || i === 7 ? '$7' : `\$${i+1}`).join(', ')})`, [animalId, viewDate, roundType, activeTab, value, note || null, currentUserId]);
            }
        },
    ...
    const signOffMutation = useMutation({
        mutationFn: async () => {
            await db.query(
                `UPDATE daily_rounds SET completed_by = $1, completed_at = now(), general_section_note = $2, updated_at = now() WHERE date = $3 AND shift = $4 AND is_deleted = false`,
                [currentUserId, generalNotes, viewDate, roundType]
            );
        },
        onSuccess: () => {
    ...
    const handleAction = (animalId: string, field: string, value: boolean, note?: string) => toggleMutation.mutate({ animalId, field, value, note });
    const toggleMob = (id: string) => { const next = new Set(expandedMobs); if (next.has(id)) next.delete(id); else next.add(id); setExpandedMobs(new Set(next)); };
    ...
    return (
        <div className="flex flex-col min-h-full bg-slate-50 relative">
            ...
            <div className="bg-white border-t border-slate-200 p-4 sticky bottom-0 z-30 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] w-full">
                <div className="max-w-[1200px] mx-auto flex flex-col md:flex-row gap-4 md:items-center">
                    <div className="w-full md:w-1/3">
                        ...
                        <div className="h-3 bg-slate-100 rounded-full overflow-hidden border border-slate-200"><div className="h-full bg-emerald-500 transition-all" style={{ width: `${progress}%` }}></div></div>
                    </div>
                    <div className="flex-1 flex gap-3">
                        ...
                        <button onClick={() => signOffMutation.mutate()} disabled={!isComplete || !signingInitials || signOffMutation.isPending} className="bg-slate-900 text-white px-6 py-3 rounded-xl font-black uppercase text-xs tracking-widest flex items-center gap-2 hover:bg-slate-800 disabled:opacity-50">
            ...
            {reportModalOpen && (
                <div className="fixed inset-0 bg-slate-900/80 z-[100] flex items-center justify-center p-4">
                    <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden">
                        <div className="p-6 border-b"><h2 className="font-black text-lg uppercase">Report Issue</h2></div>
                        <div className="p-6 space-y-4">
                            ...
                            <div className="flex gap-3">
                                ...
                                <button onClick={confirmIssue} disabled={!issueText} className="flex-1 py-3 text-white rounded-xl font-black uppercase text-[10px] disabled:opacity-50 bg-rose-600">Confirm</button>
                            </div>
                        </div>
                    </div>
                </div>
